...
const formatWithBullets = (html = "", withBullets = true) => {
  ...
  return lines
    .map(
      (line) =>
        `<div class="bullet-line" style="direction:rtl;text-align:right">
          ${withBullets ? "• " : ""}${line}
        </div>`
    )
    .join("");
 ...
 const raw = Number(item?.ingFont || 1.1);
     ...
     const fontSizeIng = 0.6 *  sizeOption?.value || 13;
     ...
     const rawIns = Number(item?.insFont || 1.1);
         ...
         const fontSizeIns = 0.6 * sizeOptionIns?.value || 13;
...
>
 ...
 <Typography variant="body1" sx={{ fontWeight: 300,fontSize: 14, color: "#000" }}>
            {item?.level || "-"}
          </Typography>
        </Stack>
