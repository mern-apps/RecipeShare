...
import Header from '../AllRecipes/Template/Header.js';
  ...
  const Categories = ({ 
    ...
    const groupcategories = useSelector(state => state.recipespage.groupcategories);

    const countallrecipes = useSelector(state => state.recipespage.countallrecipescategory);
    const allrecipes = useSelector(state => state.recipespage.allrecipescategory);
...
//check why  סלטים is checked by deafult insted of ראשונות
