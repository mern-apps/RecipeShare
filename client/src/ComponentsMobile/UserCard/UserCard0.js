...
>
  {currentUser?.description?.trim() || ' עדיין לא הוספת תיאור אישי.'}
</Typography>
  </Box>
</Box>
  ...
  >
    ...
    sx={{
      ...
      {newImages
        .filter(image => image !== placeholderImage)  // exclude placeholders
        .map((image, index) => (
          <Box
            key={index}
            sx={{
